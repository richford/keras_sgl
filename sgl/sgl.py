...
class SGLBase(object):
    """Sparse Group Lasso base object for inheritance by Classifier or Regressor
    ...
    """
    def __init__(self, dim_input, groups,
...
@registered
class SGLClassifier(SGLBase):
    """Sparse Group Lasso Logistic Regression Classifier.
    ...
    """
    def __init__(self, dim_input, n_classes, groups,
                 alpha=0.5, lambda_=0.01, n_epochs=500, batch_size=256,
                 optimizer="sgd", lr=0.01, validation_split=0.0,
                 early_stopping_patience=3, verbose=0):

        super(SGLClassifier, self).__init__(
            dim_input=dim_input, groups=groups, alpha=alpha, lambda_=lambda_,
            n_epochs=n_epochs, batch_size=batch_size, optimizer=optimizer,
            lr=lr, validation_split=validation_split,
...
@registered
class SGLRegressor(SGLBase):
    """Sparse Group Lasso Linear Regressor.
    ...
    biases_ : numpy.ndarray of shape `(n_classes, )`
        Logistic Regression biases.
    """
    def __init__(self, dim_input, groups, alpha=0.5, lambda_=0.01,
                 n_epochs=500, batch_size=256, optimizer="sgd", lr=0.01,
                 validation_split=0.0, early_stopping_patience=3, verbose=0):
    ...
    def _init_model(self):
        self.regularizer = SSGL_WeightRegularizer(
            l1_reg=self.alpha * self.lambda_,
            l2_reg=(1. - self.alpha) * self.lambda_, groups=self.groups
        )
    ...
    def predict(self, X):
        """Predict the value of samples in X.
...
@registered
class SGLMultiLayerPerceptronClassifier(SGLClassifier):
    """Sparse Group Lasso Multi Layer Perceptron classifier.
    ...
    def __init__(self, dim_input, n_classes, hidden_layers, groups,
                 alpha=0.5, lambda_=0.01, n_epochs=500,
        ...
        if len(self.hidden_layers) == 0:
            raise ValueError("No hidden layer given, you should use SGLClassifier class instead")

        super(SGLMultiLayerPerceptronClassifier, self).__init__(
            dim_input=dim_input, n_classes=n_classes, groups=groups,
            alpha=alpha, lambda_=lambda_, n_epochs=n_epochs,
...
@registered
class SGLMultiLayerPerceptronRegressor(SGLRegressor):
    """Sparse Group Lasso Multi Layer Perceptron regressor.
    ...
    def __init__(self, dim_input, n_classes, hidden_layers, groups,
        ...
        self.activation = activation
        if len(self.hidden_layers) == 0:
            raise ValueError("No hidden layer given, you should use SGLRegressor class instead")

        super(SGLMultiLayerPerceptronRegressor, self).__init__(
            dim_input=dim_input, groups=groups,
            alpha=alpha, lambda_=lambda_, n_epochs=n_epochs,
            batch_size=batch_size, optimizer=optimizer,
            validation_split=validation_split,
            early_stopping_patience=early_stopping_patience, verbose=verbose
...
class SSGL_WeightRegularizer(Regularizer):
    ...
    """
    def __init__(self, l1_reg=0., l2_reg=0., groups=None):
        self.l1_reg = l1_reg
        self.l2_reg = l2_reg
        if groups is None:
            self.groups = None
    ...
    def __call__(self, x):
        loss = K.sum(K.abs(x)) * self.l1_reg
